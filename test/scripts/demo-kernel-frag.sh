...
# demo-kernel-frag.sh -- test/demo for ccn-lite:
#   CCNx relaying via UDP sockets, where a CCN relay sits in the Linux kernel
#   and messages are fragmented (the kernel module has to reassemble them)
...
$CCNL_HOME/ccn-lite-relay -v 99 -x $UXB -u $PORTB -d $CCNL_HOME/test/ccnb 2>/tmp/b.log &
sleep 1
...
echo Fragments generated by relay B:
egrep '(bytes to fragment)|(offset now)' /tmp/b.log
