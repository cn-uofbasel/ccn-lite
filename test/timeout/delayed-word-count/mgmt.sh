...
echo "Create forwarding rule A->B for /ndn. FaceID: $FACEID"
$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-a.sock prefixreg /node/nodeA $FACEID ndn2013 \
  | $CCNL_HOME/bin/ccn-lite-ccnb2xml
...
$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-b.sock prefixreg /node/nodeA $FACEID ndn2013 \
  | $CCNL_HOME/bin/ccn-lite-ccnb2xml
...
echo "Create forwarding rule A->B for /node/nodeA. FaceID: $FACEID"
$CCNL_HOME/bin/ccn-lite-ctrl -x /tmp/mgmt-nfn-relay-c.sock prefixreg /node/nodeA $FACEID ndn2013 \
  | $CCNL_HOME/bin/ccn-lite-ccnb2xml