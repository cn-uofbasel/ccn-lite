                                 ...
                                 size_t *offset, uint8_t *buf)
{
    ...
    *offset -= mdlength; // sha256 msg digest bits, filled out later
    mdoffset = *offset;
    // mandatory
    if (ccnl_ndntlv_prependTL(NDN_TLV_SignatureValue, (uint64_t) mdlength, offset, buf) < 0) {
        return -1;
    }
    ...
    *offset -= 32;
    ...
    ccnl_hmac256_sign(keyval, 64, buf + *offset, (int) (endofsign - *offset), //fixme:type !!!
                      buf + mdoffset, &mdlength);
