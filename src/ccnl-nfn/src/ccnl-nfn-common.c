...
ccnl_nfn_freeStack(struct stack_s* s)
{
    while (s) {
        ...
        case STACK_TYPE_PREFIX:
            ccnl_prefix_free(((struct ccnl_prefix_s *)s->content));
            break;
        case STACK_TYPE_INT:
        default:
            ccnl_free(s->content);
...
ccnl_nfn_freeMachineState(struct fox_machine_state_s* f)
{
    ...
    /*while (f->prefix_mapping) {
        ...
        ccnl_prefix_free(m->key);
        ccnl_prefix_free(m->value);
    }*/
    ccnl_free(f);
    TRACEOUT();
}
                             ...
                             struct ccnl_interest_s *interest)
{
    ...
    // Search the pit for an already existing keepalive interest with this prefix.
    for (i = ccnl->pit; i; i = i->next) {
        if (!ccnl_prefix_cmp(pfx, NULL, i->pkt->pfx, CMP_EXACT)) {
            break;
        }
    }

    // If no existing keepalive interest was found, create a new one.
    if (!i) {
    ...
    // Set up relation between original and keepalive interest.
    if (i) {
