...
int main(){
    ...
    for(prefix_cmp_suite = 0; prefix_cmp_suite < 3; ++prefix_cmp_suite) {
        ...
        sprintf(testdescription,
                "testing prefix cmp with a match of 3 components for longest prefix matching with suite %s",
                ccnl_suite2str(prefix_cmp_suite));
        res = RUN_TEST(testnum, testdescription, ccnl_test_prepare_prefix_cmp_longest_match,
                       ccnl_test_run_prefix_cmp_longest_match, ccnl_test_cleanup_prefix_cmp, p1, p2);
