...
CcnContext::set802Info(const char  *srcMac, const char *dstMac)
{
    ...
    // NOTE: missing sanity check for the params correctness
...
CcnInet::initialize()
{
    ...
    /* Create UDP-tunnels for socket Faces
     */
    numUdpFaces = 0;
    basePortUdpFaces = 0;
    udpSockTbl = NULL;
...
CcnInet::whichUdpSocket (const IPvXAddress &address, const int port)
{
    ...
    DBGPRT(AUX, Warn, this->getFullName())
...
CcnInet::toLowerLayer(cMessage *msg)
{
    ...
    if ( ccnCtx->isAddressFamily(AF_INET) || ccnCtx->isAddressFamily(AF_INET6)) {
        ...
        if (s) {
            ...
            DBGPRT(EVAUX, Info, this->getFullName()) << "Sending CcnPacket over UDP socket" << std::endl;
        ...
        if ( ccnCtx->getSrcAddress802().isBroadcast() ) {
            ...
            DBGPRT(EVAUX, Info, this->getFullName()) << "Broadcasting CcnPacket over all Link Layer NICs" << std::endl;
            goto CLEANUP_AND_EXIT;
        }
        ...
        if (nicGateId != -1)
        {
            ...
            DBGPRT(EVAUX, Info, this->getFullName()) << "Sending CcnPacket via Link Layer NIC, with MAC ID="
                    << ccnCtx->getSrcAddress802().str() << std::endl;
            outPkts++;
            goto CLEANUP_AND_EXIT;
...
CcnInet::toUpperLayer(cMessage *msg)
{
    ...
    {
        send(msg, "upperOut");
        DBGPRT(EVAUX, Info, this->getFullName())
                << "Passing CcnAppMessage to layer above."
                << std::endl;
    }
    else {
        delete msg;
        DBGPRT(EVAUX, Warn, this->getFullName())
    ...
    else if (dynamic_cast<CcnPacket *>(msg))
    {
        ...
        if (msg->arrivedOn("ifIn") ) {
            ...
            if ( ! (ccnCtx = new CcnContext(AF_PACKET)) ) {
                ...
                DBGPRT(AUX, Err, this->getFullName())
             ...
             */
            DBGPRT(EVAUX, Info, this->getFullName())
                    << "Received CcnPacket from the Link Layer with MAC endpoints (src,dst): "
