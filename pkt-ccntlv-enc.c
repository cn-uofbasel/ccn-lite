...
ccnl_ccntlv_prependChunkName(struct ccnl_prefix_s *name,
    ...
    if (chunknum && *chunknum >= 0 && ccnl_ccntlv_prependNetworkVarInt(
                               CCNX_TLV_N_Chunk, *chunknum, offset, buf) < 0)
            return -1;
...
// write given prefix *before* buf[offs], adjust offset and return bytes used
int
                               ...
                               int *offset, int *contentpos, unsigned char *buf)
{
    ...
    oldoffset = *offset;
    len = ccnl_ccntlv_fillContent(name, payload, paylen, chunknum,
                                  lastchunknum, offset, contentpos, buf);
